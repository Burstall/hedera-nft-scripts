...
async function pullTransactions(pathUrl, wallet, accountBalanceMap = new Map()) {
	try {
		...
		for (let i = 0; i < data.transactions.length; i++) {
			...
			if (tx.transfers) {
				...
				for (let t = 0; t < tx.transfers.length; t++) {
					...
					amt += Number(new Hbar(-transfer.amount, HbarUnit.Tinybar).toBigNumber());
					// only look at credits to the account
					if (excludeList.includes(account)) continue;
					if (account != wallet && amt > 0 && account != '0.0.800') {
		...
		if (data.links.next) {
			return await pullTransactions(data.links.next, wallet, accountBalanceMap);
		}
...
async function main() {
	if (getArgFlag('h')) {
		console.log('usage: node checkHbarRaffleEntries.mjs -cost XX -start <epoch> -end <epoch> -wallet 0.0.ZZZ -token 0.0.QQQ [-exclude 0.0.XX,0.0.WW]');
	...
	if (!wallet || !startTime || !endTime || !cost) {
	...
	const acctBalMap = await pullTransactions(pathUrl, wallet);
	...
	console.log('\ttotal hbar collected: ', totalAmount);
	...
	try {
		...
		const filename = `./hbar-raffles-${timestamp}.txt`;
